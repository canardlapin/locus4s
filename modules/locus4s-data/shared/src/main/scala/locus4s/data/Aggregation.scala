...
object Aggregation:
  ...
  def foldMapByWith[X, Y, A, M](
    ...
    val accumulated =
      freshAccumulators(grouping.to.size, empty)
    grouping.foreachDefined: (source, target) =>
      accumulated(target.ordinal) = combine(
        accumulated(target.ordinal),
        contribution(field(source))
      )
  ...
  /** Certified-partition overload; aggregation delegates to its partial map. */
  def foldMapBy[X, Y, A, M](
      grouping: PartialSurjection[X, Y],
      field: Field[X, A]
  )(
      empty: => M
  )(
    ...
    foldMapBy(grouping.toPartialMap, field)(empty)(contribution)(combine)
  ...
  def foldMapByChecked[X, FX, Y, A, M](
    ...
    foldMapByChecked(grouping.toPartialMap, field)(empty)(contribution)(
      combine
    )
  ...
  /** User-facing spelling for partial-map pushforward. */
  def pushForward[X, Y, A, M](
      grouping: PartialMap[X, Y],
      field: Field[X, A]
  )(
  ...
  /** User-facing spelling for certified partial-surjection pushforward. */
  def pushForward[X, Y, A, M](
      grouping: PartialSurjection[X, Y],
      field: Field[X, A]
  )(
